...
def db_add_user(username, password, group_name=None):
    ...
    try:
        ...
        print("adding user,", db_encrypt_data(username))
        cursor.execute("INSERT INTO users (username, password_hash, group_name) VALUES (?, ?, ?)", (db_encrypt_data(username), password_hash, db_encrypt_data(group_name)))
...
def change_directory_permissions(dir_name, group_ids):
    try:
        ...
        cursor.execute("SELECT dir_id FROM directories WHERE dir_name = ?", (db_encrypt_data(dir_name)))
        dir_id = cursor.fetchone()
        ...
        if dir_id:            
            ...
            cursor.execute("UPDATE directories SET permissions = ? WHERE dir_id = ?", (db_encrypt_data(permissions), dir_id))
